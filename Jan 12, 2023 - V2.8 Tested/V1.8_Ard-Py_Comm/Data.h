...
// Function will update the Voltage and Current of the System
String updateVAsenseSystem() {
  // Read the Analog Input
  for (SensorIterator = 0; SensorIterator < 10; ++SensorIterator) {
    adc_value = analogRead(VSENSE_SYSTEM);
   
    // Determine voltage at ADC input
    adc_voltage  = (adc_value * 4.7) / 1024.0; 
  ...
  //systemCurrent = analogRead(AsenseSystem);
  //systemPower = systemVoltage * systemCurrent;
  return(String(AVGsystemVoltage));
}

//Function will update the Voltage and Current of the Panel
// void updateVAsensePanel() {
//   panelVoltage = analogRead(VsensePanel);
//   panelCurrent = analogRead(AsenseSystem);
